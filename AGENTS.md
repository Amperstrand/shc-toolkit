...
## Lessons Learned (2026-08-31 session)

### 28. Domain-dark outages: prove the vantage before blaming your DNS; nostr cannot route around SHC's own domain
`sovereignhybridcompute.com` went globally unresolvable for ~90 minutes (SERVFAIL / EDE-22 "no reachable authority" — a lame delegation, SHC-side). Three traps in sequence: (1) the lab's plain-DNS path is allowlist-filtered (answers debian.org, blackholes SHC), which looked like the cause — resolve the ambiguity with **cert-verified DoH** (issuer must be Google Trust Services etc.; a middlebox can't forge it) and a **GitHub Actions vantage** no local middlebox touches; (2) the DoH SERVFAIL was genuine — two independent resolvers + GitHub agreed; (3) the IP workaround needs a recorded origin IP — none existed anywhere locally (swept 48 repos); post-recovery we learned it: **`23.182.128.4`** — next time, `curl --resolve blesta...:443:23.182.128.4 <spec-url> -I` then `/etc/hosts` (runbook on #40). **Nostr does not bypass the outage**: the operate-lane exchange endpoint and every API call live on the dead domain; relays + `shc mail` + signing all survive, SHC management does not. `shc-dns-watch.yml` (added this session) monitors from GitHub and auto-closes the tracking issue on recovery — its close step needed `GH_REPO` (no checkout ⇒ no repo context for `gh`) and lowercase coercion (`gh --json state` returns `OPEN`).
