...
def main():
    ...
    try:
        ...
        if not wait_ssh(host1):
            print("VM #1 SSH failed")
            return 5
        print(
            f"VM #1 ready at {host1}; polling cloud-init status every 10s for 5 min..."
        )
        ...
        for i in range(30):  # 5 min, every 10s
            ...
            if (
                "done" in status.lower()
                or "error" in status.lower()
                or "disabled" in status.lower()
            ):
        ...
        hardening = {
            ...
            "password_auth": ssh(
                host1,
                "sudo grep -i '^PasswordAuthentication' /etc/ssh/sshd_config 2>&1 || echo 'not set'",
            ),
            "permit_root": ssh(
                host1,
                "sudo grep -i '^PermitRootLogin' /etc/ssh/sshd_config 2>&1 || echo 'not set'",
            ),
        }
        ...
        keys1 = ssh(
            host1,
            "sudo ssh-keygen -l -f /etc/ssh/ssh_host_ed25519_key.pub 2>&1; sudo ssh-keygen -l -f /etc/ssh/ssh_host_rsa_key.pub 2>&1; sudo cat /etc/ssh/ssh_host_ed25519_key.pub 2>&1",
        )
        ...
        if not wait_ssh(host2):
            ...
            # Compare
            print("\n=== host key comparison ===")
            fp1 = [entry for entry in keys1.splitlines() if "SHA256" in entry]
            fp2 = [entry for entry in keys2.splitlines() if "SHA256" in entry]
            print(f"  VM #1 fingerprints: {fp1}")
            print(f"  VM #2 fingerprints: {fp2}")
            if fp1 and fp2 and fp1 == fp2:
                verdict = (
                    "DUPLICATE: both VMs have identical SSH host key fingerprints!"
                )
                print(f"  ⚠️  {verdict}")
            elif fp1 and fp2:
