...
def main() -> int:
    ...
    if not Path(ROOTFS_SRC).exists():
        print(f"ERROR: rootfs not found at {ROOTFS_SRC}", file=sys.stderr)
        return 2
    ...
    print(
        f"Rootfs template: {ROOTFS_SRC} ({Path(ROOTFS_SRC).stat().st_size // 1024 // 1024} MB)"
    )
    print()
    ...
    with ThreadPoolExecutor(max_workers=args.count) as pool:
        ...
        for fut in as_completed(futures):
            ...
            print(
                f"  μVM #{r['idx']:02d}: {status}  boot→init={boot_t}s  wall={r.get('wall_s', '?')}s"
            )
    ...
    boots = [
        r["boot_to_init_s"] for r in results if r.get("boot_to_init_s") is not None
    ]
    if boots:
        print(
            f"  boot→init: min={min(boots):.3f}s  max={max(boots):.3f}s  avg={sum(boots) / len(boots):.3f}s"
        )
