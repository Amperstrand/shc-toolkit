...
### Fixed
- **Reinstall template matrix: all non-default templates fail upstream (drip findings, 2026-09-11).** 26-template sweep on a Katy box produced three distinct failure classes: (a) `not_found: Template not found` — firecracker, pve-ve (not in the reinstall-side universe, which has 1 template vs the 32 in the ordering catalog); (b) `upstream_failure: Reinstall failed` — freebsd14, netbsd10, openbsd79 and every subsequent attempt (the hypervisor rejects the reinstall); (c) accepted-but-unchanged — openwrt (the API returned success but `os_template` stayed `debian13-cloud`). Only the current template (debian13-cloud) reinstalls reliably. This is a provider-side limitation worth escalating alongside the other findings.

### Added
- **`shc_toolkit/console_client.py` — the headless Blesta console client: VM screen + keyboard with no browser, no Playwright, no portal login. LIVE-PROVEN 2026-09-11 (42KB screenshot of a running VM's tty, 1280×800).** Reverses SHC's `proxmoxbridge`: `POST /proxmoxbridge/console/bootstrap` with the `create_console_session` JWT (cookies from the response MUST ride the WebSocket upgrade — "Missing bridge session" otherwise; relative `ws_url` resolves against the origin with wss) → RFB over WebSocket with the returned VNC password. `asyncvnc` supplies RFB (injectable-opener design), bridged onto `websockets` via StreamReader/Writer shims — `screenshot()`/`type_text()` sync facades included; offline mock-RFB test pins handshake + zlib framebuffer decode. Operational contracts encoded: one console session per VM with newest-mint-wins (minting revokes the prior token instantly — never interleave mints), tokens are single-use, and **the VM must be RUNNING** — a stopped VM bootstraps fine but the WS closes cleanly with zero RFB bytes (initially misread as a server-side bridge outage; it was the drip sweep stranding the box stopped, since fixed). Requires the `console` extra.
...
### Fixed
- **egress-probe cancel step + dev-zone-watch tracepath hang.** The cancel step's `python3 -c "…${{…}}…"` was a YAML plain scalar — folding collapsed the newlines and the VM leaked (sid 2527, cancelled manually within minutes; now a heredoc reading `$EGRESS_SID` from step env). `tracepath -n` in the watch's `target_ip` probe hangs 30s+ on unroutable targets and failed the whole run despite a PASS TCP verdict (13:28 run: OPEN in 0.07s, red conclusion) — now guarded, TCP/22 stays the signal.
- **api-drift llms baseline: diff the full corpus, not the lean llms.txt (#48 root cause).** The drift job fetched the lean `/user-api/llms.txt` while the tracked baseline (`.omo/llms.txt`) is the full `agent-skills/llms-full.txt` corpus — two different artifacts, so every dispatched run would flag false drift (the lean file is regenerated on spec rebuilds: its header moved 2.4.15→2.4.24 on 2026-09-10 while `op_fingerprint`/`contractFingerprint` — the actual semantic signal — were unchanged; that mismatch created #48). The fetch now pulls `llms-full.txt`, the artifact AGENTS.md baselines; the committed baseline matches it byte-for-byte. Lean-file regeneration is docs-side noise, not API drift — the OpenAPI diff and catalog check remain the API-drift signals.
- **ansible-e2e: twelve stacked defects cleared; remaining failure is an external NoDNS outage (2026-09-10 arc).** Layers this arc: (10) the workflow never overrode `certbot_email` — ACME (even staging) rejects the `you@example.com` placeholder; now the CI account's nomail mailbox. (11) the certbot auth hook was the last pre-nostr-sdk-0.45 code in the repo (`Keys.from_bech32`, `sign_with_keys`, sync `Client` calls, kwargs `EventBuilder`) — migrated to the proven 0.45 flow, offline signing-verified, repo-wide sweep confirms nothing pre-0.45 remains. (12) the hook published DNS records as JSON content — the toolkit's proven shape is 11-element `record` **tags** (`build_record_tag`); the hook now mirrors it exactly. **Terminal finding:** with all twelve layers fixed, certbot still fails because `nodns.shop` answers EVERY name (including garbage) with a wildcard A to ns1 itself (46.224.104.12) — the nodns.bot materializes nothing; the NoDNS lane is externally down. The e2e stays honestly red until that recovers (the toolkit's own `publish_dns_records` "succeeds" into the same void — the July end-to-end verification predates the outage). NoDNS remains documented-experimental; no SHC API involvement.
...
### Added
- **Multi-zone live smoke — `scripts/live_smoke.py --zone all` (monthly, ~$0.06/run): one VM per catalog line with zone-dependent criteria.** The smoke now orders the cheapest size on EVERY line (nvme/hdd/ssd/dev) instead of one fixed NVMe VM. **Stable lines** (Katy): hard pass/fail (order, active+IP, ssh_key, template, TCP/22, cancel). **Watch lines** (Cherryvale ssd/dev): identical checks run but failures WARN and never fail the run — the smoke watches them recover, it doesn't gate on them (#39). Stock/unavailability rejections warn for any line (capacity is not a defect). Built-in **Katy→Cherryvale bastion canary** for watch lines (one shared Katy bastion, SSH `/dev/tcp` probe): `unreachable` is today's expected state; `open` means SHC added the internal route. Helpers `sizes.smallest_size_for_line`/`is_watch_line` are pinned by unit tests incl. an all-lines-present guard against catalog drift. `--zone <line>` still smokes a single line for debugging. Also: the ansible `shc_vm` role now passes the SSH key at ORDER time (cloud-init seed — reliable) instead of relying on apply-live, which proved unreliable (API records the key, guest never gets it; live-verified 2026-09-10).
- **`docs/zone-reliability.md` — the zone/reliability map + vantage-check runbook.** Consolidates the 2026-09-09/10 investigation: facility table (module groups, nested-KVM, BGP state), the 4-vantage reachability matrix for the dev zone (EU lab + 15 check-host.net nodes ALL dark **including their US node**; GitHub/Azure OPEN in 0.07s; cross-zone from a Katy VM = `Network is unreachable` — the gateway has no route), the per-zone predicted-reliability verdicts, and the reusable runbook: RIPEstat prefix-overview (zero-cost BGP check), check-host.net 16-node TCP matrix, `dev-zone-watch target_ip` dispatch (GitHub/Azure vantage for any live IP), order-time-key bastion cross-zone test, and the two-vantage verdict rule. Answers the operational question: **no, a Katy bastion does not unlock the dev zone** — and the only live path to Cherryvale dev is GitHub runners, which is one routing change from dark.
- **`dev-zone-watch.yml` gains a `target_ip` dispatch input — on-demand GitHub/US vantage for any live IP.** Probes TCP/22 + tracepath from the runner without ordering a VM or commenting; the building block of the vantage-check runbook above.
- **`shc order --verify-reachability` / `--verify-timeout` (issue #44 gap 2).** Post-provision reachability gate: waits for `service_status=active` + IP (never `provisioning_state` — it lies forever), then polls TCP/22 up to `--verify-timeout` (default 120s). If port 22 never opens — the #39 pattern, billing-active-but-unroutable — the VM is **cancelled** (immediate, prorated refund, 1h minimum; never stop: stopped VMs still bill) and the command exits 1; a failed auto-cancel prints a loud `STILL BILLING` pointer instead of swallowing. Works in both the plain and `--pay` order paths. Encodes the pay → reachability-test-first loop the campaigns ran by hand; pairs with the order-time unstable-zone refusal that guards the front half.
- **`dev-zone-watch.yml` — weekly automated #39 evidence trail.** `scripts/dev-zone-probe.py` from GitHub's vantage every Monday 05:47 UTC (+ dispatch): broken → one evidence comment on #39 (deduped against fresh bot comments; manual dispatches never comment, so testing is spam-free), PASS → evidence comment only. **No auto-close:** reachability proved vantage-dependent live on 2026-09-09 — GitHub/US reached TCP/22 in 0s (sid 2481) while the EU lab stayed dark on the same IP 6 minutes later (sid 2482); the first version auto-closed #39 on that single-vantage PASS and was reverted to comment-only the same day (#39 reopened with the asymmetry evidence — the zone is partially repaired: US routes attach, EU routes don't).
- **Zero-cost confirm-gate smoke in `api-drift.yml` (issue #45 corroboration).** New job: order submit with `confirm=False` must raise 409 `confirmation_required` **carrying a `confirmation_id`** — nothing is created, nothing is spent. Catches gate-shape drift the OpenAPI diff cannot see (the 2026-09-06 server-side 500s on properly-formed confirmations had an unchanged spec); gate verified healthy live 2026-09-09 (probe passed, and a full confirmed order+cancel succeeded — sid 2476).
- **`docs/campaign-lifecycle.md` — the ephemeral-VM recipe as a guide.** The order → provision → run → exfiltrate → close loop with receipts, the guard stack in firing order (`--allow-unstable-zone` refusal → `--verify-reachability` → `--reap` → self-destruct timer → daily reaper → session-end dry-run), and what "closed with receipts" means. Extracted from the 2026-09 lightning-playground campaigns (issue #44 gap 3, recipe-first as planned).

### Changed
- **`ansible-demo-` joins the reaper's default reap prefixes; e2e gains the ssh-keygen step the play needs.** The 09-09 dispatch leaked sid 2483: the play ordered the VM, then died at the SSH-key task (runners have no `~/.ssh/id_ed25519.pub` — the workflow now generates one), and the hostname matched NO reap prefix so every cleanup layer treated it as permanent. Cancelled manually within the hour ($0.25 refund). With the prefix added, layer-1 reaping catches E2E VMs immediately regardless of age.
- **`ansible-e2e` retargeted to Katy NVMe — E2E tests the toolkit, not the zone; plus the real #46 root cause fixed.** Default `shc_vm_size` is now `nvme-1c-4gb` (the playbook runs Caddy + certbot; no nested KVM needed) with a `vm_size` dispatch input for deliberate Dev-zone debugging. The 09-07 failure (#46) looked like zone collateral, but a dispatch run with the NVMe default failed identically at the ORDER task: `default(\".\")` inside the roles' Jinja — invalid template syntax under the ansible-core the workflow installs unpinned — aborted the play before anything touched SHC. All four occurrences (shc_vm ×3, nodns ×1) now use `default('.')`; render verified with Jinja2 for both defined/undefined paths.
- **`shc order` now REFUSES flagged facilities instead of warning (fail-early).** Ordering an `ssd-*`/`dev-*` size (Cherryvale, KS — g7) exits 1 before submitting: those orders go billing-active within ~80s and then never attach to the network (issue #39 — probe FAILed again live 2026-09-04: active + IP, ICMP silent, TCP/22 closed 120s), so waiting out the provisioning timeout cannot help and only burns money. `--allow-unstable-zone` opts in for deliberate zone debugging; `--dry-run` only previews (keeps the warning); `scripts/dev-zone-probe.py` remains the long-wait debugging tool (`--timeout`, `--net-timeout`). `shc contextvm` orders pass the same guard (`compute.py`). Consumer-side counterpart pushed the same evening: microfips `14679bc` (SSH-banner readiness + cancel-on-timeout instead of waiting for the daily reaper).

### Added
- **AGENTS.md: "Operating VMs with the shc CLI" operator section (2026-09-13 lessons).** Agents burned multiple round-trips on an invalid size name (`--size starter` fails; only spec names like `nvme-1c-4gb` resolve, despite the `--size` help text advertising the friendly words), the order/pay two-step (no VM until the invoice is paid), the `shc pay` confirmation gate (409 `confirmation_required` + `X-User-Api-Confirm` resend; owner must explicitly approve the payment first), and `ssh-keys` arity (per-VM, takes `service_id`; no global key upload exists). The section pins each fact to source (`sizes.py`, `cli.py`, `client.py`), adds the standing nvme-only zone rule, cost/shutdown-billing facts, a command map, sharp edges (idempotency-key charset, `--dry-run`, reap semantics, gated vs routine ops), and a six-step disposable-VM session recipe. README Quick Start now points agents at it.
- **`catalog.json` — machine-readable catalog artifact (issue #37, dry/static).** `scripts/generate-catalog-json.py` dumps the static catalog model into a stable JSON document for cross-language consumers (Go provider, Pulumi bridge): per-line storefront triples, per-package specs/spec-name/daily-price/pricing-id/option-ids/templates, schema `shc-catalog/1` with api+toolkit versions. Stdlib-only (loads `catalog_model` standalone — no httpx), deterministic (sorted keys, byte-identical re-runs), `--check` flag doubles as a drift gate. Committed at repo root; shape pinned by `tests/test_catalog_artifact.py`. **Consumed**: terraform-provider-shc now embeds the artifact (`provider/catalog.json`) and derives its size map + storefront triples from it — cross-repo parity by construction. The cross-repo audit gained a **Catalog Artifact Parity** check (byte-compares the provider's embedded copy against a fresh generation; a stale copy fails the audit), and `_parse_go_sizes` reads the artifact instead of regexing Go literals.
...
### Fixed
- **`sign_operate_grant()` now backdates `nbf` by 30s.** The operate-lane exchange rejects grants with `nbf == signing-time` — live-earned 2026-08-31 during the post-outage lane verification ("Grant not yet valid"; the NIP-98 auth and grant signature were fine, the server enforces a clock-skew margin). Full two-party lane re-verified live after the fix: lease issued (`scope=operate`, `area=vm:2242`, 900s), vm-scoped read OK, spend 403. Two more server contracts documented on the way: the signer must differ from the agent, and the grant signer's ownership of the VM is checked at exchange (AGENTS lesson 29).
...
- **`shc list -o table` shows stopped-but-billable zombies at a glance (issue #38 remainder).** Every non-canceled VM gets a runtime probe (the reap-proven `get_vm_summary` path, fail-soft to `?`); stopped/shutdown rows carry `← STILL BILLING`, and a footer states the counts plus the fact that cancel — not shutdown — ends billing. The old `runtime_status` column was dead weight (the list payload never carries it per the OpenAPI schema). Machine output (`json`/`yaml`) is unchanged — no per-VM enrichment calls. **`shc info -o table` gains a billing block**: daily rate, the full-rate-while-stopped warning, an accrued-to-date estimate (rate × days since creation — SHC exposes no cost ledger), and the cancel pointer.
...
### Changed
- **Doc audit: exact test counts removed from living docs.** The pinned counts went stale three times within a week (288 → 360 → 380 → actual 364) — the exact failure mode AGENTS.md warns about. README/ROADMAP/AGENTS now describe the suite qualitatively and point at `pytest --collect-only -q` for the current number; the dated CHANGELOG history keeps its release-time counts as immutable records. Same sweep: ROADMAP toolkit version 2.4.24.0 → 2.4.24.3, README CI workflow list corrected (claimed 7, listed 9, actual 10 — now countless), Ansible E2E weekly → monthly, AGENTS integration-test trigger "push" → tag + monthly (matching the CI trigger policy).
