...
### Changed
- **`ansible-e2e` retargeted to Katy NVMe — E2E tests the toolkit, not the zone.** Default `shc_vm_size` is now `nvme-1c-4gb` (the playbook runs Caddy + certbot; no nested KVM needed) with a `vm_size` dispatch input for deliberate Dev-zone debugging; scheduled runs were ordering into the unroutable Cherryvale facility (#39) and failing at the SSH-wait step (#46, logs expired but the failed step is "Fail job if provision failed" and today's probe re-confirmed the zone dark). Failure-issue text now points at #39/#dev-zone-watch instead of the resolved #28.
