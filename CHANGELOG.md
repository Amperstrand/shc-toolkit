...
- **`dev-zone-watch.yml` — weekly automated #39 evidence trail.** `scripts/dev-zone-probe.py` from GitHub's vantage every Monday 05:47 UTC (+ dispatch): broken → one evidence comment on #39 (deduped against fresh bot comments; manual dispatches never comment, so testing is spam-free), recovered → evidence comment + auto-close, mirroring the proven `shc-dns-watch.yml` recovery pattern (incl. its `GH_REPO` + lowercase-state lessons). Probe VM is always cancelled (~$0.01-0.24/run).
