...
class SshKeyEntry:
    ...
    service_id: int
    ssh_key: str
    """ The single-line public key. Multi-key historical values are split into one entry per line. """
    key_fingerprint: None | str
    """ Server-computed SHA-256 fingerprint in OpenSSH form (`SHA256:` + base64 with padding stripped). Pass this
    verbatim to `DELETE /ssh-key` to remove the entry. May be `null` for keys that fail to parse server-side. """
