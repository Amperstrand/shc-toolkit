...
|------|-------|----|-------------|
| shc-toolkit | unit + mypy | dispatch + tag + monthly (REST+MCP) + OpenAPI drift + MCP drift + cross-repo parity + typecheck + ansible + publish | Spec-encoding sizes, config options, cost audit, MCP transport (157 server tools, 157 TOOL_MAP entries, 100% of curated 35 x-shc-core wrapped, 100% of all MCP-exposed ops), v2.4.24, static catalog model (replaces 10.3 MB fetch), catalog generator, auto-generated client (932 files, 729 attrs models, 148 endpoints), cloud-init REST wrappers, full confirmation-flow coverage, close_support_ticket confirmation fix, idempotency keys, 408 retry, confirmation flow |
...
### Toolkit version: 2.4.24.3 (API-aligned: `<SHC_API_VERSION>.<toolkit_patch>`)
...
- **MCP coverage**: `TOOL_MAP` 157 entries wrapping 157/157 (100%) of MCP-exposed ops and 35/35 (100%) of the curated `x-shc-core` subset.
...
| MCP transport | 157 TOOL_MAP entries, 100% of curated 35 x-shc-core wrapped, 100% of all 157 MCP-exposed ops | N/A | N/A |
